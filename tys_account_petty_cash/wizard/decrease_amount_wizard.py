...
class DecreaseAmountPettyCashWizard(models.TransientModel):
    ...
    amount_decrease_petty_cash = fields.Monetary('Monto Disminuido')
    ...
    amount_new = fields.Monetary('Nuevo Monto')

    @api.onchange('amount_decrease_petty_cash')
    def _get_amount_new(self):
        if self.amount_decrease_petty_cash > self.amount:
            raise Warning(_('el Monto Disminuido no puede ser mayor al Monto Actual'))
        else:
    ...
    def validate_petty_cash(self,petc_ids):
        ...
        invoice_petty_cash = self.env['invoice.petty.cash'].search([('code', '=', petty_cash.id)])
        if invoice_petty_cash:
            for a in invoice_petty_cash:
                a.write({'apertura': petty_cash.petty_cash_amount_open - self.amount_decrease_petty_cash,
                          'disponible': a.disponible - self.amount_decrease_petty_cash,
                          })
        ...
        if move_line_id1 and move_line_id2:
            petty_cash.write({'petty_cash_status': 'validate',
                   ...
                   'disponible': petty_cash.disponible - self.amount_decrease_petty_cash,
                   'name': petty_cash.name})
